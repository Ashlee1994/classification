...
class ResNet(object):
  ...
  def _build_model(self):
    ...
    for i in six.moves.range(1, self.args.num_residual_units):
    ...
    with tf.variable_scope('unit_last'):
      ...
      x = self._relu(x, self.args.relu_leakiness)
      x = self._global_avg_pool(x)

    with tf.variable_scope('logit'):
      logits = self._fully_connected(x, self.args.num_classes)
      self.predictions = tf.nn.softmax(logits)
      self.accuracy =  tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.predictions,1),tf.argmax(self.labels, 1)), tf.float32))
  ...
  def _build_train_op(self):
    ...
    self.lrn_rate = tf.constant(self.args.lrn_rate, tf.float32)
  ...
  def _decay(self):
    ...
    return tf.multiply(self.args.weight_decay_rate, tf.add_n(costs))
