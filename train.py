...
from utils import load_train_6
...
def train():
    ...
    train_x, train_y, test_x, test_y = load_train_6()
    ...
    with tf.Session() as sess:
        ...
        for e in range(args.num_epochs):
            ...
            # print("test_train_pred: ", test_train_pred)
            print("avg = %.6f , min = %.6f, max = %.6f "% (np.mean(test_train_pred,axis = 0)[0],np.min(test_train_pred, axis = 0)[0],np.max(test_train_pred,axis = 0)[0]))    
            accuracy_train = np.sum(np.equal(np.argmax(test_train_pred, axis=1),np.argmax(test_train_y, axis=1)))/len(test_train_y)
            ...
            if e % 5 == 0:
                ...
                print("avg = %.6f , min = %.6f, max = %.6f "% (np.mean(test_pred,axis = 0)[0],np.min(test_pred, axis = 0)[0],np.max(test_pred,axis = 0)[0]))
                accuracy = np.sum(np.equal(np.argmax(test_pred, axis=1),np.argmax(test_y, axis=1)))/len(test_y)
